...
uses  {$ifdef MSWINDOWS}
      ...
      tterm_api, tterm_common, tterm_logger;

type  tOnIdleHandler      = procedure of object;
...
type  tCommandLine        = class(tObject)
      ...
      public
        ...
        function    readlncommand(const aprompt: ansistring; amasked: boolean; aidleproc: tOnIdleHandler = nil): ansistring;
...
function  executeconsolecommand(acommand: pAnsiChar): boolean; stdcall;
...
function tCommandLine.readlncommand(const aprompt: ansistring; amasked: boolean; aidleproc: tOnIdleHandler): ansistring;
...
var cmd : ansistring;
begin
  if assigned(commandline) and (buflen > 0) then begin
    cmd:= commandline.readlncommand(ansistring(prompt), masked);
    result:= math.min(length(cmd), buflen);
...
begin result:= readconsolecommand(prompt, masked, buf, buflen); end;

function executeconsolecommand(acommand: pAnsiChar): boolean;
begin
...
begin
  helpfilename:= format('%s%s', [ExeFilePath, ChangeFileExt(ExeFileName, '.hlp')]);
  if assigned(commandline) then
    if fileexists(helpfilename) then begin
      assignfile(txt, helpfilename); reset(txt);
      commandline.lock;
      try
        gotoxy(1,wherey); clreol;
        while not eof(txt) do begin
          readln(txt,st);
          if commandline.UseOEM then writeln(ANSIToOEM(st)) else writeln(st); 
        end;
        commandline.outputcommandline;
      finally commandline.unlock; end;
      closefile(txt);
    end else log('HELP: file %s not found', [helpfilename]);
end;
