...
CREATE VIEW "public"."Daily_Finres" AS  SELECT t1.date,
    t1.tpid,
    t1.balance,
    (t1.value)::integer AS "Value",
    (t1.finres)::integer AS finres,
    (sum(t2.finres))::integer AS summ_finres
   FROM ("Finres_History" t1
     LEFT JOIN "Finres_History" t2 ON (((t1.tpid = t2.tpid) AND (t1.date >= t2.date))))
  GROUP BY t1.date, t1.tpid, t1.balance, t1.value, t1.finres;

-- ----------------------------
-- Alter sequences owned by
-- ----------------------------
SELECT setval('"public"."messages_id_seq"', 127364, true);

-- ----------------------------
-- Alter sequences owned by
-- ----------------------------
SELECT setval('"public"."myorders_transactionid_seq"', 1732590, true);
