/*
 ...
 Date: 21/10/2021 11:08:23
*/


-- ----------------------------
-- Sequence structure for asset_move_id_seq
-- ----------------------------
DROP SEQUENCE IF EXISTS "public"."asset_move_id_seq";
CREATE SEQUENCE "public"."asset_move_id_seq" 
INCREMENT 1
MINVALUE  1
MAXVALUE 2147483647
START 1
...
-- ----------------------------
-- Table structure for _import_hkf
-- ----------------------------
DROP TABLE IF EXISTS "public"."_import_hkf";
CREATE TABLE "public"."_import_hkf" (
...
-- ----------------------------
-- Table structure for _tmp_valchange
-- ----------------------------
DROP TABLE IF EXISTS "public"."_tmp_valchange";
CREATE TABLE "public"."_tmp_valchange" (
  "tpid" int4 NOT NULL,
  "securityid" int4 NOT NULL,
  "tradeno" int8 NOT NULL,
  "oldqty" int4 NOT NULL,
  "aqty" int4 NOT NULL,
  "avalue" float4 NOT NULL,
  "oldvalue" float4 NOT NULL,
  "newvalue" float4 NOT NULL
)
;
...
-- ----------------------------
-- Table structure for asset_move
-- ----------------------------
DROP TABLE IF EXISTS "public"."asset_move";
CREATE TABLE "public"."asset_move" (
  "id" int4 DEFAULT nextval('asset_move_id_seq'::regclass),
  "date" timestamp(6),
  "value" float8,
...
-- ----------------------------
-- Table structure for tp_list_service
-- ----------------------------
DROP TABLE IF EXISTS "public"."tp_list_service";
CREATE TABLE "public"."tp_list_service" (
  "tpid" int4,
...
CREATE OR REPLACE FUNCTION "public"."AddUpdateBalance"("atradeno" int8, "atransaction" int4, "acode" varchar, "aquantity" int4, "avalue" float4)
  RETURNS "pg_catalog"."int4" AS $BODY$
	...
	DECLARE vOldQty     int;
--	declare vOldBalance float;
--	declare vNewBalance float;
BEGIN
	...
	--vOldBalance = (SELECT "value" FROM public.Balance WHERE TPId = vTPId AND SecurityId = vSecurityId);
...
--	vNewBalance = (SELECT "value" FROM public.Balance WHERE TPId = vTPId AND SecurityId = vSecurityId);
	
	/*
		insert into "public"."_tmp_valchange"
		(tpid,securityid,tradeno,oldqty,aqty,avalue,oldvalue,newvalue)
		values
		(vTPId, vSecurityId, atradeno, vOldQty, aquantity, avalue, vOldbalance, vNewBalance);
	*/
...
DECLARE vbasesecid int;
DECLARE vfullpdkf float;
DECLARE vbalance int;
DECLARE vrepo_perc float;
    ...
    LOOP
			vbasesecid = (SELECT sec_id FROM "public".tp_sec WHERE tp_id = vtp.tpid AND sec_type ='B');
			vfullpdkf = (SELECT COALESCE(pd_kf * quote2 / 10, 1) FROM "FR_RevalAndTradesToNow"(vtp.tpid, vdt) WHERE pdtosecid = vbasesecid);			
			vbalance = (SELECT qty2 FROM "FR_RevalAndTradesToNow"(vtp.tpid, vdt) WHERE sec_id = vbasesecid);
			vrepo_perc = (SELECT "RepoPerc" FROM "public".tp_basis_count WHERE "TPId" = vtp.tpid);
			
			
			SELECT SUM(fullprofit * (CASE WHEN sec_id = vbasesecid THEN vfullpdkf ELSE 1 END)) INTO STRICT vfr FROM "FR_RevalAndTradesToNow"(vtp.tpid, vdt);
			SELECT SUM("Val") INTO STRICT vval FROM "FR_BalanceToTime"(vdtend) WHERE tpid = vtp.tpid AND securityid < 1000 ;
		
			RAISE NOTICE 'TP % (%) Secid=% fr=%  val=%  pdkf=% balance=% repo_perc=%', vtp.tpid, vtp.name, vbasesecid, vfr, vval, vfullpdkf, vbalance, vrepo_perc;
			INSERT INTO "public"."Finres_History"("date", "tpid", finres, "value", balance, repo_perc)
					VALUES(vdt, vtp.tpid, COALESCE(vfr, 0), COALESCE(vval, 0), COALESCE(vbalance,0), COALESCE(vrepo_perc,0));  
    END LOOP;
...
CREATE OR REPLACE FUNCTION "public"."FR_RevalAndTradesToDate"("atpid" int4, "adt1" timestamp, "adt2" timestamp)
  RETURNS TABLE("sec_id" int4, "code" varchar, "lotsize" int4, "pdtosecid" int4, "pd_kf" float4, "qty1" int4, "quote1" float8, "qty2" int4, "quote2" float8, "reval" numeric, "qtybuy" int8, "valbuy" numeric, "qtysell" int8, "valsell" numeric, "tradeprofit" numeric, "fullprofit" numeric) AS $BODY$ 
...
CREATE OR REPLACE FUNCTION "public"."FR_RevalToDate"("atpid" int4, "adt1" timestamp, "adt2" timestamp)
  RETURNS TABLE("sec_id" int4, "code" varchar, "lotsize" int4, "pdtosecid" int4, "pd_kf" float4, "qty1" int4, "quote1" float8, "qty2" int4, "quote2" float8, "reval" numeric) AS $BODY$ 
...
-- ----------------------------
-- Function structure for _User_ChangeKf
-- ----------------------------
DROP FUNCTION IF EXISTS "public"."_User_ChangeKf"("deltaVolOff" int4, "KfV" float8, "deltaM" float8);
CREATE OR REPLACE FUNCTION "public"."_User_ChangeKf"("deltaVolOff" int4, "KfV" float8, "deltaM" float8)
  RETURNS "pg_catalog"."int4" AS $BODY$
BEGIN

	UPDATE "public".securities SET 
      volOff = volOff + "deltaVolOff"
      ,vmin = (vmin * "KfV")::int
      ,mhigh = mhigh + "deltaM"
      ,vmax = (vmax * "KfV")::int
      ,mlow = mlow + "deltaM" 
...
-- ----------------------------
-- Function structure for _User_DeleteTp
-- ----------------------------
DROP FUNCTION IF EXISTS "public"."_User_DeleteTp"("atpid" int4);
CREATE OR REPLACE FUNCTION "public"."_User_DeleteTp"("atpid" int4)
  RETURNS "pg_catalog"."int4" AS $BODY$
BEGIN

	DELETE FROM "public".tp WHERE tpid = atpid;
	DELETE FROM "public".tp_sec WHERE tp_id = atpid;
...
-- ----------------------------
-- Function structure for _User_Import_Hkf
-- ----------------------------
DROP FUNCTION IF EXISTS "public"."_User_Import_Hkf"("atpid" int4, "aaccountid" int4);
CREATE OR REPLACE FUNCTION "public"."_User_Import_Hkf"("atpid" int4, "aaccountid" int4)
  RETURNS "pg_catalog"."int4" AS $BODY$
	DECLARE vtpcount	int;
	DECLARE vnullcount	int;	
BEGIN

	vtpcount = (select count(*) from tp where tpid = atpid);
	
	IF (vtpcount > 0) THEN
		raise notice 'Filling TP %', aTPId;
		vnullcount = (select count(*) from _import_hkf as i left join securities as s on (TRIM(i.code)=s.code) 
					where s.securityid is null);				
	  IF (vnullcount > 0) THEN
			raise notice 'Some security codes are incorrect';
		ELSE
			delete from tp_sec where tp_id = atpid AND sec_type = 'H';
			insert into tp_sec (tp_id, sec_id, accountid, sec_type, hedge_kf, pd_kf, pdtosecid, p2p_kf)			
				(
					select atpid AS "tp_id", s.securityid AS "sec_id", aaccountid AS "accountid", 'H' AS "sec_type",
							i.hedgekf AS "hedge_kf", 0 AS "pd_kf", 0 AS "pdtosecid", 1 AS "p2p_kf"
							FROM _import_hkf AS i LEFT JOIN securities AS s ON (TRIM(i.code)=s.code)
				);
			raise notice 'HedgeKf Imported';
		END IF;	
	ELSE
		raise notice 'No TP found';
	END IF;
...
begin
	...
	vValFinish = (CASE WHEN (v_stockid = 4 AND v_code LIKE '%Si-%') THEN v_price * v_quantity * 1000 ELSE v_value END);
	...
	UPDATE public.Trades
	   SET transaction = v_transaction
		  ...
		  ,price = v_price
		  ,quantity = v_quantity
		  ,value = vValFinish --(CASE WHEN (v_stockid = 4 AND v_code LIKE '%Si-%') THEN v_price * v_quantity * 1000 ELSE v_value END)
		  ,accr = v_accr
		  ,clientid = v_clientid
	...
	--	PERFORM OffTradeQty(v_tradeno);
		...
		PERFORM "public"."AddUpdateBalance"(v_tradeno, v_transaction, v_code, QtyChange, vValFinish); 
				--		CASE WHEN (v_stockid = 4 AND v_code LIKE '%Si-%') THEN v_price * v_quantity * 1000 ELSE v_value END);
...
CREATE VIEW "public"."TP_Basis_Count_Full" AS  SELECT "TP_Basis_Count_tmp".tpid,
  ...
  WHERE ("TP_Basis_Count_tmp"."IsActive" = '1'::"bit");

-- ----------------------------
-- View structure for TP_Balances_2_old
-- ----------------------------
DROP VIEW IF EXISTS "public"."TP_Balances_2_old";
CREATE VIEW "public"."TP_Balances_2_old" AS  SELECT t.tpid,
    t.name,
    (- fbb.qty) AS qty,
...
CREATE VIEW "public"."TP_Balances_2" AS  SELECT t.tpid,
    ...
    (fbp.basismean)::numeric(16,2) AS basismean
   FROM ((tp t
     LEFT JOIN ( SELECT "Full_Balances_2_2".tp_id,
            "Full_Balances_2_2".qty
           FROM "Full_Balances_2_2"
          WHERE (("Full_Balances_2_2".sec_type)::text = 'B'::text)) fbb ON ((t.tpid = fbb.tp_id)))
     LEFT JOIN ( SELECT t_1.tp_id,
            sum(((((t_1.price)::double precision *
                CASE
                    WHEN (t_1.kffrompd IS NOT NULL) THEN ((t_1.pdprice)::double precision * t_1.kffrompd)
                    ELSE t_1.pdps_def
                END) * t_1.hedge_kf) * (t_1.mult)::double precision)) AS basismean
           FROM ( SELECT fb1.tp_id,
                    fb1.name,
                    fb1.sec_id,
                    fb1.code,
                    fb1.lotsize,
                    ...
                    fb1.pdtosecid,
                    fb1.pd_kf,
                    fb2.price AS pdprice,
                    (fb2.pd_kf / ((10 * fb2.lotsize))::double precision) AS kffrompd,
                        CASE
                            WHEN ((fb1.sec_type)::text = 'B'::text) THEN '-1'::integer
                            WHEN ((fb1.sec_type)::text = 'H'::text) THEN 1
                            ELSE 0
                        END AS mult
                   FROM ("Full_Balances_2_2" fb1
                     LEFT JOIN "Full_Balances_2_2" fb2 ON (((fb1.sec_id = fb2.pdtosecid) AND (fb1.tp_id = fb2.tp_id))))) t_1
          GROUP BY t_1.tp_id) fbp ON ((t.tpid = fbp.tp_id)))
  WHERE (t.isactive = '1'::"bit");
...
-- ----------------------------
-- View structure for Full_Balances_2_2
-- ----------------------------
DROP VIEW IF EXISTS "public"."Full_Balances_2_2";
CREATE VIEW "public"."Full_Balances_2_2" AS  SELECT tps.tp_id,
    t.name,
    tps.sec_id,
    s.code,
    s.lotsize,
    ...
    (
        CASE
            WHEN (COALESCE(b.quantity, 0) <> 0) THEN (COALESCE(b.value, (0)::real) / (COALESCE(b.quantity, 0))::double precision)
            ELSE (0)::double precision
        END)::numeric(16,6) AS price,
    tps.sec_type,
    tps.hedge_kf,
    s.pdps_def,
    tps.pdtosecid,
...
-- ----------------------------
-- View structure for FinresWithRepo
-- ----------------------------
DROP VIEW IF EXISTS "public"."FinresWithRepo";
CREATE VIEW "public"."FinresWithRepo" AS  SELECT "Finres_History".date,
    "Finres_History".tpid,
    "Finres_History".balance,
    "Finres_History".finres,
    (- ((("Finres_History".value * "Finres_History".repo_perc) / (365)::double precision) / (100)::double precision)) AS repoprofit
   FROM "Finres_History";

-- ----------------------------
-- View structure for FinresWithRepo_2
-- ----------------------------
DROP VIEW IF EXISTS "public"."FinresWithRepo_2";
CREATE VIEW "public"."FinresWithRepo_2" AS  SELECT "Finres_History".date,
    "Finres_History".tpid,
    "Finres_History".balance,
    "Finres_History".finres,
    COALESCE((- ((("Finres_History".value * "Finres_History".repo_perc) / (365)::double precision) / (100)::double precision)), (0)::double precision) AS repoprofit
   FROM "Finres_History";

-- ----------------------------
-- View structure for Daily_Finres
-- ----------------------------
DROP VIEW IF EXISTS "public"."Daily_Finres";
CREATE VIEW "public"."Daily_Finres" AS  SELECT t1.date,
    t1.tpid,
    t1.balance,
    (t1.finres)::integer AS finres,
    (t1.repoprofit)::integer AS repoprofit,
    ((t1.finres + t1.repoprofit))::integer AS total,
    (sum(t2.finres))::integer AS summ_finres,
    (sum(t2.repoprofit))::integer AS summ_repo,
    (sum((t2.finres + t2.repoprofit)))::integer AS summ_total
   FROM ("FinresWithRepo_2" t1
     LEFT JOIN "FinresWithRepo_2" t2 ON (((t1.tpid = t2.tpid) AND (t1.date >= t2.date))))
  GROUP BY t1.date, t1.tpid, t1.balance, t1.repoprofit, t1.finres;

-- ----------------------------
-- View structure for AssetMoves_Daily
-- ----------------------------
DROP VIEW IF EXISTS "public"."AssetMoves_Daily";
CREATE VIEW "public"."AssetMoves_Daily" AS  SELECT asset_move.date,
    sum(asset_move.value) AS finres
   FROM asset_move
  GROUP BY asset_move.date;

-- ----------------------------
-- View structure for AssetMoves_Full
-- ----------------------------
DROP VIEW IF EXISTS "public"."AssetMoves_Full";
CREATE VIEW "public"."AssetMoves_Full" AS  SELECT t1.date,
    9999999 AS tpid,
    0 AS balance,
    t1.finres,
    0 AS repoprofit,
    t1.finres AS total,
    sum(t2.finres) AS summ_finres,
    0 AS summ_repo,
    sum(t2.finres) AS summ_total
   FROM ("AssetMoves_Daily" t1
     LEFT JOIN "AssetMoves_Daily" t2 ON ((t1.date >= t2.date)))
  GROUP BY t1.date, t1.finres;

-- ----------------------------
-- View structure for TPListService
-- ----------------------------
DROP VIEW IF EXISTS "public"."TPListService";
CREATE VIEW "public"."TPListService" AS  SELECT tp.tpid,
    ((('('::text || ((tp.tpid)::character varying)::text) || ') '::text) || (tp.name)::text) AS fullname
   FROM tp
  WHERE (tp.isactive = '1'::"bit")
UNION
 SELECT tp_list_service.tpid,
    ((('('::text || ((tp_list_service.tpid)::character varying)::text) || ') '::text) || (tp_list_service.fullname)::text) AS fullname
   FROM tp_list_service
  ORDER BY 1;

-- ----------------------------
-- View structure for SummFinres_Full
-- ----------------------------
DROP VIEW IF EXISTS "public"."SummFinres_Full";
CREATE VIEW "public"."SummFinres_Full" AS  SELECT t.date,
    sum(t.finres) AS finres,
    sum(t.repoprofit) AS repoprofit,
    sum(t.total) AS total,
    sum(t.summ_finres) AS summ_finres,
    sum(t.summ_repo) AS summ_repo,
    sum(t.summ_total) AS summ_total
   FROM ( SELECT "Daily_Finres".date,
            "Daily_Finres".tpid,
            "Daily_Finres".balance,
            "Daily_Finres".finres,
            "Daily_Finres".repoprofit,
            "Daily_Finres".total,
            "Daily_Finres".summ_finres,
            "Daily_Finres".summ_repo,
            "Daily_Finres".summ_total
           FROM "Daily_Finres"
        UNION
         SELECT "AssetMoves_Full".date,
            "AssetMoves_Full".tpid,
            "AssetMoves_Full".balance,
            "AssetMoves_Full".finres,
            "AssetMoves_Full".repoprofit,
            "AssetMoves_Full".total,
            "AssetMoves_Full".summ_finres,
            "AssetMoves_Full".summ_repo,
            "AssetMoves_Full".summ_total
           FROM "AssetMoves_Full") t
  GROUP BY t.date;

-- ----------------------------
-- Alter sequences owned by
-- ----------------------------
SELECT setval('"public"."asset_move_id_seq"', 13, true);

-- ----------------------------
-- Alter sequences owned by
-- ----------------------------
SELECT setval('"public"."messages_id_seq"', 301322, true);

-- ----------------------------
-- Alter sequences owned by
-- ----------------------------
SELECT setval('"public"."myorders_transactionid_seq"', 2924869, true);
