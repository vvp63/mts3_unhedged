...
uses {$ifdef useexceptionhandler} ExcHandler, {$endif}
     ...
     patsystems_common, patsystems_commandparser, patsystems_tables,
     PATSINTF;
  ...
  if assigned(contract_list) then begin
    ...
    if (strlen(@contract.ContractName) > 0) then begin
      ...
      if (ptAddOrder(@neworder, @orderid) = ptSuccess) then begin
        ...
        with res do begin accepted:= soRejected; ExtNumber:= 0; TEReply:= 'PATSYSTEMS: Failed to place order!'; end;
      end;

    end else with res do begin
      accepted:= soRejected; ExtNumber:= 0; TEReply:= 'PATSYSTEMS: Invalid instrument!';
    end;
  end else with res do begin
    accepted:= soRejected; ExtNumber:= 0; TEReply:= 'PATSYSTEMS: Internal error!';
  end;
end;
...
begin
  with res do begin
    accepted:= soRejected; ExtNumber:= 0; TEReply:= 'PATSYSTEMS: MoveOrder is not supported!';
  end;
end;
...
begin
  ...
  if (ptCancelOrder(@order_id) = ptSuccess) then begin
    ...
    with res do begin
      accepted:= soRejected; ExtNumber:= 0; TEReply:= 'PATSYSTEMS: Unable to drop order!';
    end;
  end;
end;
