...
const PlugName  = 'sqlevents';

const ExeFileName          : ansistring = 'sqlevents.dll';
      IniFileName          : ansistring = 'sqlevents.ini';
...
begin
  if assigned(server_api) and assigned(server_api^.LogEvent) then
    server_api^.LogEvent(pAnsiChar(format('SQLEVENTS: ' + aevent, aparams)));
end;

procedure log(const aevent: ansistring);
begin
  if assigned(server_api) and assigned(server_api^.LogEvent) then
    server_api^.LogEvent(pAnsiChar('SQLEVENTS: ' + aevent));
end;
